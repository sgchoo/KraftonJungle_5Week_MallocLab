...
static char *heap_listp;        // 항상 next 가용 블록을 가리키는 정적 전역 변수

static void add_freeBlock(void *bp);
static void replace_list(void *bp);
...
static void replace_list(void *bp)
{
    if(bp == heap_listp)
    {
        heap_listp = NEXT_FREEBLOCK(heap_listp);
        return;
    }
...
static void place(void *bp, size_t asize)
{
    ...
    if ((csize - asize) >= (2 * DSIZE)) // 차이가 최소 블록 크기 16보다 같거나 크면 분할
    {
        ...
        if (NEXT_FREEBLOCK(bp) != NULL) // 다음 가용 블록이 있을 경우만
            PREV_FREEBLOCK(NEXT_FREEBLOCK(bp)) = bp;
    }
    else
    {
